 ...
 * @param int $postID
 */
function sage_set_post_views($postID) {
    $count_key = 'sage_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    $count_update = '_sage_post_views_count_time';
...
function sage_get_post_views($postID) {
    $count_key = 'sage_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if ($count == '') {
        delete_post_meta($postID, $count_key);
...
function sage_track_post_views($post_id) {
    if (!is_single())
        return;
    if (empty($post_id)) {
        global $post;
    ...
    echo "<!-- mfunc mysecretcode sage_set_post_views($post_id); -->";
    sage_set_post_views(get_the_ID());
    echo "<!-- /mfunc mysecretcode -->";
}

add_action('wp_head', 'sage_track_post_views');

function sage_admin_bar_post_views($wp_admin_bar) {
    ...
    if (is_single()) {
        ...
        $args = array(
            'id' => 'custom-views',
            'title' => '<span class="ab-icon dashicons-visibility"></span> ' . sage_get_post_views($post_id) . ' ' . __('Views', 'sage'),
...
add_action('admin_bar_menu', 'sage_admin_bar_post_views', 90);
